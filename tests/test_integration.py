...
class TestServiceIntegration:
    ...
    def test_create_simple_service(self, mock_run_command):
        ...
        service = Service(
            "test-app",
            service_location=ServiceLocation.TEST,
            auto_start=False,
    ...
    def test_create_service_with_timer(self, mock_run_command):
        ...
        with open(timer_file_path, "r") as f:
            ...
            )  # systemd expects lowercase boolean values
            assert "[Install]" in content
    ...
    def test_update_existing_service(self, mock_run_command):
        ...
        with open(service_file_path, "r") as f:
            ...
            assert (
                "WantedBy=multi-user.target" in content
    ...
    def test_boolean_conversion_to_lowercase(self):
        ...
        try:
            ...
            with open(timer_path, "r") as f:
                ...
                assert (
                    "Persistent=true" in content
                ), f"Expected lowercase 'true', got content: {content}"
                assert (
                    "WakeSystem=false" in content
                ), f"Expected lowercase 'false', got content: {content}"
                # Ensure uppercase versions are NOT present
                assert (
                    "Persistent=True" not in content
                ), "Found uppercase 'True' instead of lowercase 'true'"
                assert (
                    "WakeSystem=False" not in content
                ), "Found uppercase 'False' instead of lowercase 'false'"
